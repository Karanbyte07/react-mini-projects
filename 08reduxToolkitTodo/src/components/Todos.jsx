...
function Todos() {
  ...
  const dispatch = useDispatch() // Get the dispatch function for dispatching actions
  const [editingId, setEditingId] = useState(null) 
  const [editText, setEditText] = useState('')

  //it will handle the edit functionality
  const handleEdit = (todo) =>{
    setEditingId(todo.id) // set the id of the todo being edited
    setEditText(todo.text)// set the current text of the todo
  }

  // it will handle the save functionality
  const handleSave = (id) => {
    if(editText.trim()){ // trim is used to remove whitespace so that empty todos are not saved
